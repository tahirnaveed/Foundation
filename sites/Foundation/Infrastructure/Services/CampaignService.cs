    ...
    public class CampaignService : ICampaignService
    {
        ...
        public void UpdatePoint(int point)
        {
            try
            {
                ...
                // Update recipient
                if (currentRecipientListId != -1 && long.TryParse(currentPoints[0], out long oldBonusPoints) && long.TryParse(currentPoints[1], out long oldScore))
                {
                    AddOrUpdateRecipient(new Dictionary<string, string>
                    {
                        { bonusPoints, (oldBonusPoints + point).ToString() },
                        { score, (oldScore + point).ToString() },
                        { "Email", email }
                    }, currentRecipientListId);
                }
            }
            catch (Exception ex)
            {
