...
namespace Foundation.Features.Blocks
{
    ...
    public class RecentPageCategoryRecommendationController : BlockController<RecentPageCategoryRecommendationBlock>
    {
        ...
        [HttpGet]
        public override ActionResult Index(RecentPageCategoryRecommendationBlock currentContent)
        {
            ...
            var rootFilterId = currentContent.FilterRoot != null ? currentContent.FilterRoot.ID : ContentReference.RootPage.ID;
            var pageTypesFilterId = currentContent.FilterTypes?.Split(',').ToList().Select(x => int.Parse(x));
            ...
            if (pageTypesFilterId != null && pageTypesFilterId.Any())
            {
            ...
            var model = new RecentPageCategoryRecommendationViewModel(currentContent)
            {
            ...
            if (!pages.Any())
            {
                if (ContentReference.IsNullOrEmpty(currentContent.FilterRoot))
                {
                    return PartialView("~/Features/Blocks/Views/RecentPageCategoryRecommendation.cshtml", model);
                }

                pages = _contentLoader.GetChildren<FoundationPageData>(currentContent.FilterRoot).Where(x => x.ContentLink.ID != _pageRouteHelper.Page.ContentLink.ID).ToList();
            }

            var pageCount = pages.Count;
            var count = Math.Max(currentContent.NumberOfRecommendations, pageCount);
            if (count > currentContent.NumberOfRecommendations)
            {
                count = currentContent.NumberOfRecommendations;
            }
