...
namespace Foundation.Features.Blocks
{
    ...
    public class MembershipDisplayBlockController : SocialBlockController<MembershipDisplayBlock>
    {
        ...
        /// </summary>
        /// <param name="currentContent">The current block instance.</param>
        [HttpGet]
        public override ActionResult Index(MembershipDisplayBlock currentContent)
        {
            //Populate model to pass to the membership display view
            var membershipDisplayBlockModel = new MembershipDisplayBlockViewModel(currentContent);

            //Retrieve the group id assigned to the block and populate the member list
            try
            {
                var group = _communityRepository.Get(currentContent.GroupName);
                ...
                if (group != null)
                {
                    ...
                    {
                        CommunityId = groupId,
                        PageSize = currentContent.NumberOfMembers
                    };
