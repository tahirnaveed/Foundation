...
namespace Foundation.Features.Blocks
{
    ...
    public class CarouselBlockController : BlockController<CarouselBlock>
    {
        ...
        [HttpGet]
        public override ActionResult Index(CarouselBlock currentContent)
        {
            var model = new CarouselBlockViewModel(currentContent);

            if (currentContent.CarouselItems != null)
            {
                foreach (var contentAreaItem in currentContent.CarouselItems.FilteredItems)
                {
