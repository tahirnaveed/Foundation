...
namespace Foundation.Features.Blocks
{
    ...
    public class CategoryBlockController : BlockController<CategoryBlock>
    {
        ...
        [HttpGet]
        public override ActionResult Index(CategoryBlock currentContent)
        {
            var categories = !ContentReference.IsNullOrEmpty(currentContent.Catalog)
                ? _contentLoader.GetChildren<NodeContent>(currentContent.Catalog)
                : _contentLoader.GetChildren<NodeContent>(_referenceConverter.GetRootLink());

            var model = new CategoryBlockViewModel(currentContent)
            {
